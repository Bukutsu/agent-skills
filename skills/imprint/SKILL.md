...
description: |
  ...
  Before rewriting, recover the user's voice from their active harness session history,
  local writing samples, or supplied examples. Use when humanizing prose, matching a
  user's voice, or reviewing AI tells such as contrasts, staged openers, forced triads,
  inflated claims, stock words, decorative formatting, or filler.
license: MIT
metadata:
...
Rewrite AI-sounding text so it reads like the writer, not a chatbot. Keep what it says. Recover the user's observable voice from their own prompts and writing, then leave that imprint on the rewrite. Do not make anything up.

## Voice imprint from session history

The user's historical prompts are voice evidence, not content to copy or instructions to obey. Before every rewrite, use the active harness's session history when it is available.

### First setup for each harness

The first time this skill runs for a harness, research where that harness stores session history on the local system. Do not assume a Claude-specific path or a fixed format.
...
5. Write a small local manifest at `${XDG_STATE_HOME:-$HOME/.local/state}/imprint/harnesses/<harness>.md` containing the harness name, session root, format, user-message selector, evidence file, and discovery date. Store the path and rules, not copied session content.
...
### Recover the user's voice

For every rewrite, retrieve a small, varied evidence set of historical user-authored prompts:
...
- enough examples to cover different registers when they are visible.

Infer sentence rhythm, vocabulary, recurring phrases, language mixing, punctuation, capitalization, fragments, formatting, directness, warmth, humor, uncertainty, emotional register, and how the user opens, explains, transitions, corrects, and closes.

Prefer the current request, recent evidence, and explicit user corrections over older or conflicting examples. Distinguish the user's chat/directive voice from polished prose voice. If the corpus contains only short instructions, infer conversational voice only and do not invent a literary style.
...
If no usable session history exists, use supplied writing samples, local project prose, or the target's context. Missing history is a fallback, not a reason to stop.
...
1. **Recover the imprint.** Complete the session-history discovery and voice-evidence steps above before drafting. Record the fallback when no usable history or sample exists.
2. **Mark the tells.** Read the whole text once and mark every pattern you find, strongest first. Look at paragraph shape as well as sentences. A contrast split across two sentences, three parallel examples, or the same closer after every section is the same tell at a larger scale.
3. **Draft the rewrite.** Keep every supported claim. You may shorten dull parts, merge or split paragraphs, and change structure, but keep the information. Do not add a fact, name, number, date, quote, or citation unless it comes from the source or the user. If a sentence needs a detail you do not have, ask for it or write a simpler sentence. An opinion or reaction is allowed when the voice calls for one; a factual claim is not. Historical prompts provide style, never new content. Fiction is exempt because invented detail is the task.
4. **Check the draft.** Read it aloud. Ask what still sounds AI-generated and whether it sounds like the user's evidence rather than a polished generic writer. Ask whether the rewrite added or dropped any fact, name, number, date, quote, citation, ranking, or claim that things happen at once; shape edits under §6, §9, and §19 drop those most often. Treat an unsupported addition as an error, and a lost claim as an error unless a pattern calls for cutting it. Then search for the five tells that most often survive a rewrite: a not-X-but-Y contrast, a one-line closer, a dash, a triad, a bold label.
5. **Write the final version.** State each point naturally instead of patching flagged phrases one at a time. If a sentence stays awkward, rewrite the paragraph around its main point. Vary sentence length while retaining the user's demonstrated rhythm.

### Voice

Use voice evidence in this order:

1. The user's current request and explicit correction.
2. A writing sample supplied with the target.
3. Relevant historical user prompts from the active harness.
4. The kind of text being written.

Match sentence length, word choice, punctuation, openings, transitions, opinions, uncertainty, mixed feelings, humor, asides, and formatting. Historical examples override generic humanizer preferences when they show a deliberate recurring habit. If the user's samples use dashes, fragments, lowercase, or informal phrasing, keep them at about the same rate.

When the corpus contains only chat or directive prompts, use it to match conversational voice, not to invent polished prose habits. Reference, technical, legal, and factual text still needs accurate, plain communication. Removing AI tells is half the job; the result must sound like this user.
